...
public:
    ...
    /*  multi-layer network constructor */
    NodeGeneticNeuralNetwork(double learningRate_,
                             double momentum_,
                             const size_t numInputs,
                             const size_t numOutputs,
...
class NGNNEvolver {
public:
    NGNNEvolver(NGNN* network_,
                DataLoader<double>* dataLoader_);

    void evolve(size_t numTrainingRounds,
                size_t numEvaluationRounds,
                double MSEConstraint,
                double mutationThreshold,
                double learningSpeedEpsilon);

    void mutate(size_t populationSize,
...
private:
    ...
    double getConnectionHandicap(NGNN* n);
};
